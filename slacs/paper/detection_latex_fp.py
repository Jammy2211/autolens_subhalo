...
evidence_bpl_dict_file = path.join(json_path, "bpl", f"evidence_grid_dict.json")
...
evidence_clumps_dict_file = path.join(json_path, "clumps", f"evidence_grid_dict.json")
...
slacs_keys = util.slacs_keys_success_list_from()
light_fp = util.slacs_light_fp_from()
light_drop = util.slacs_light_drop_from()
source_fp = util.slacs_source_fp_from()
source_drop = util.slacs_source_drop_from()
bpl_drop = util.slacs_bpl_drop_from()
...
for i, key in enumerate(slacs_keys):
    ...
    if key in ["slacs0728+3835", "slacs0959+0410", "slacs1029+0420"]:
        evidence_bpl = None
