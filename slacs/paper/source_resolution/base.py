...
plot_path = path.join(workspace_path, "paper", "images", "source_resolution_all")
...
for search, fit_imaging in zip(search_gen, fit_imaging_gen):
    ...
    #     output=aplt.Output(
    #         path=plot_path, filename=f"{filename}_base_image_lens_sub", format=["png", "pdf"], format_folder=True
    #     ),
    # )
    #
    ...
    # mat_plot_2d = aplt.MatPlot2D(
    #     title=aplt.Title(
    #         label=f"{lens_name.upper()} Model Lensed Source"
    #     ),
    #     ylabel=aplt.YLabel(labelpad=-2.0),
    # #    cmap=aplt.Cmap(norm="log", vmin=1e-2, vmax=1.0),
    #     output=aplt.Output(
    #         path=plot_path, filename=f"{filename}_base_model_lensed_image", format=["png", "pdf"], format_folder=True
    #     ),
    # )
    #
    # fit_imaging_plotter = aplt.FitImagingPlotter(
    ...
    #     output=aplt.Output(
    #         path=plot_path, filename=f"{filename}_base_norm", format=["png", "pdf"], format_folder=True
    #     ),
    # )
    #
    # fit_imaging_plotter = aplt.FitImagingPlotter(
    ...
    mat_plot_2d = aplt.MatPlot2D(
       # axis=aplt.Axis(extent=[-0.5, 0.3, -0.5, 0.3]),
        title=aplt.Title(
            label=f"{lens_name.upper()} Source Reconstruction"
        ),
        ylabel=aplt.YLabel(labelpad=-2.0),
        cmap=aplt.Cmap(vmax=0.8),
        output=aplt.Output(
            path=plot_path, filename=f"{filename}_base_source_reconstruction", format=["png", "pdf"], format_folder=True
        ),
    )
